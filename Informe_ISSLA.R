...
#Conclusiones años 2020-2021

#1º Selecciono solo los poligonos y años 2020-2021
accidentes_en_poligonos=subset(delta_poligonos, !is.na(delta_poligonos$ubicacion))
accidentes_en_poligonos_17_19=subset(accidentes_en_poligonos, year==2020|year==2021)
...
empr_trab_accs$Nº_de_trabajadores=tabla_2017_2019[1:4,5]
porcentaje=round((empr_trab_accs[1:4,3]/empr_trab_accs[4,3]), 2)
empr_trab_accs$trabajadores=percent(porcentaje)

empr_trab_accs$Nº_de_accidentes=tabla_17_19_itinere[1:4,5]
porcentaje=round((empr_trab_accs[1:4,5]/empr_trab_accs[4,5]), 2)
empr_trab_accs$accidentes=percent(porcentaje)
...
ylim.sup=max(aux.dat[1:3,1:2])*1.2
aux.ylim=c(0,ylim.sup)
bp=barplot(t(aux.dat[1:3,1:2]), beside=TRUE, ylim=aux.ylim,legend = TRUE, args.legend = list(bty = "n", x = "topright", cex=0.75, inset = c(- 0.05, 0)), cex.names = 0.75, col = c("steelblue","brown2") )
#dev.print(pdf, 'Informe_3.11_ESTUDIO_DE_LA_ACCIDENTABILIDAD_POR_TAMAÑO.pdf' ,  height=10, width=10 )
...
Ubicaciones=round((ubic_trab_accs[1:5,3]/ubic_trab_accs[5,3]), 2)*100
Accidentes=round((ubic_trab_accs[1:5,5]/ubic_trab_accs[5,5]), 2)*100
aux.dat=data.frame(Ubicaciones, Accidentes)
row.names(aux.dat)=row.names(ubic_trab_accs)
aux.dat=as.matrix(aux.dat)

ylim.sup=max(aux.dat[1:4,1:2])*1.2
aux.ylim=c(0,ylim.sup)
bp=barplot(t(aux.dat[1:4,1:2]), beside=TRUE, ylim=aux.ylim,legend = TRUE, args.legend = list(bty = "n", x = "topright", cex=0.75, inset = c(- 0.05, 0)),cex.names = 0.75, col = c("steelblue","brown2") )
#dev.print(pdf, 'Informe_3.12_ESTUDIO_DE_ACCIDENTABILIDAD_POR_UBICACIÓN.pdf' ,  height=10, width=10 )
